...
with tf.Session() as sess:
    ...
    for epoch in range(5):
        ...
        for i in range(17):
            ...
            #print(data[i * 24 + 72] == data[i * 24 + 73])
            # for data in train_meo_data:
            #     temp_x.extend(np.reshape(data[i * 24 : i * 24 + 72], (-1)))
            #     temp_y.extend(data[i * 24 + 72])
            temp_prev = np.array(temp_x)             
            for k in range(48):
                #print(np.array(temp_x) == temp_prev)
                #print(k, temp_x[:20], temp_x[-20:])
                feed_dict = {model.x_:[temp_x], model.y_:[temp_y], model.keep_prob:0.}
                predict, loss = sess.run([model.y_predict, model.loss], feed_dict)
                #print(k, predict[0][-6:], len(temp))
                ...
                for s in range(35):
                    ...
                    l = predict[0][6 * s : 6 * (s + 1)]
                    # for ll in range(6):
                    #     l[ll] = l[ll] * (1. + (random.random() - 0.5) * 0.1)
                    temp.extend(l)
