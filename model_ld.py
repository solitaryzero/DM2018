...
class Model:
    def __init__(self,
        ...
        self.x_ = tf.placeholder(tf.float32, [None, 13 * 24 * 3 * 3])
        #self.x2_ = tf.placeholder(tf.float32, [None, 840 * 5, 6])
        self.y_ = tf.placeholder(tf.float32, [None, 39])
        self.keep_prob = tf.placeholder(tf.float32)
        self.learning_rate = learning_rate
        self.W_1 = tf.Variable(tf.random_normal([2808, 500]), trainable=is_train)
        self.W_2 = tf.Variable(tf.random_normal([500, 39]), trainable=is_train)
        self.b_1 = tf.Variable(tf.random_normal([500]),trainable=is_train)
        self.b_2 = tf.Variable(tf.random_normal([39]),trainable=is_train)
        ...
        self.train_op = tf.train.AdamOptimizer(1.5e-2).minimize(self.loss)  # Use Adam Optimizer
