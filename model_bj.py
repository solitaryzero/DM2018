...
class Model:
    def __init__(self,
        ...
        self.x_ = tf.placeholder(tf.float32, [None, 35 * 24 * 3 * 6])
        #self.x2_ = tf.placeholder(tf.float32, [None, 840 * 5, 6])
        self.y_ = tf.placeholder(tf.float32, [None, 210])
        self.keep_prob = tf.placeholder(tf.float32)
        self.learning_rate = learning_rate
        self.W_1 = tf.Variable(tf.random_normal([15120, 1000]), trainable=is_train)
        self.W_2 = tf.Variable(tf.random_normal([1000, 210]), trainable=is_train)
        self.b_1 = tf.Variable(tf.random_normal([1000]),trainable=is_train)
        self.b_2 = tf.Variable(tf.random_normal([210]),trainable=is_train)

        # TODO:  implement input -- Linear -- BN -- ReLU -- Linear -- loss
        #        the 10-class prediction output is named as "logits"
        fc1 = tf.add(tf.matmul(self.x_, self.W_1), self.b_1)
        fc1 = batch_normalization_layer(fc1, 1000, is_train)
        fc1 = tf.nn.relu(fc1)
        self.y_predict = tf.add(tf.matmul(fc1, self.W_2), self.b_2)
        
        self.loss = tf.reduce_mean(tf.square(self.y_predict - self.y_))  
        
        self.train_op = tf.train.AdamOptimizer(1e-1).minimize(self.loss)  # Use Adam Optimizer
